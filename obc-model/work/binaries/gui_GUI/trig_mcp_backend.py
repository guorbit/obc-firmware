...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_trig_mcp
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_trig_mcp
    tcId = i_gui_RI_trig_mcp
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global trig_mcp_via_shared_lib
    shared_lib = True
    trig_mcp_via_shared_lib = dll.gui_PI_trig_mcp
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_trig_mcp(tc)
            except IOError:
                raise
    elif udp:
