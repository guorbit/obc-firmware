...
def setMsgQ():
    global tmId
    tmId = PythonController.i_send_tm_mcp


def setUDP():
    global tmId
    from _InterfaceEnum import i_gui_PI_send_tm_mcp
    tmId = i_gui_PI_send_tm_mcp


def send_tm_mcp(tm_ptr, size):
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.Mode_Change_Packet_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def decode_ACN(ACN_encodedBuffer):
    ...
    decoded_value = ASN1.Mode_Change_Packet()
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.Mode_Change_Packet_REQUIRED_BYTES_FOR_ENCODING)
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ send-tm-mcp ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received send-tm-mcp with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected send-tm-mcp (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print('Received other (%s), but still waiting for send-tm-mcp') % str(msgId)
            Q.task_done()


def fromPysideToASN1(_):
    print('DEPRECATED call to "fromPysideToASN1"')
    # Create a native ASN.1 variable of type Mode_Change_Packet
    # send_tm_mcp = ASN1.Mode_Change_Packet()
    return None

def typeInstance():
    # Create an intance of an ASN.1 ctype
    return ASN1.Mode_Change_Packet()
