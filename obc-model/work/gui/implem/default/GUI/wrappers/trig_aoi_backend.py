...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_trig_aoi
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_trig_aoi
    tcId = i_gui_RI_trig_aoi
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global trig_aoi_via_shared_lib
    shared_lib = True
    trig_aoi_via_shared_lib = dll.gui_PI_trig_aoi
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_trig_aoi(tc)
            except IOError:
                raise
    elif udp:
