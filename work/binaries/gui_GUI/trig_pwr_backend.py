...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_trig_pwr
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_trig_pwr
    tcId = i_gui_RI_trig_pwr
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global trig_pwr_via_shared_lib
    shared_lib = True
    trig_pwr_via_shared_lib = dll.gui_PI_trig_pwr
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.T_Null_Record_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def decode_ACN(ACN_encodedBuffer):
    ...
    decoded_value = ASN1.T_Null_Record()
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.T_Null_Record_REQUIRED_BYTES_FOR_ENCODING)
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_trig_pwr(tc)
            except IOError:
                raise
    elif udp:
